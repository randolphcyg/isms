...
func (m *IsmsSoftware) validate(all bool) error {
	...
	for idx, item := range m.GetIndustryDetails() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, IsmsSoftwareValidationError{
						field:  fmt.Sprintf("IndustryDetails[%v]", idx),
			...
			if err := v.Validate(); err != nil {
				return IsmsSoftwareValidationError{
					field:  fmt.Sprintf("IndustryDetails[%v]", idx),
					reason: "embedded message failed validation",
					cause:  err,
				}
